...
{
    _x params ["_squadName", "_group", "_currentStrength", "_maxStrength", "_reinforcing"];
    
    if (!isNull _group) then {
        private _aliveCount = {alive _x} count units _group;
        private _totalCount = count units _group;
        private _woundedCount = {alive _x && damage _x > 0.3} count units _group;
        ...
        _squadList lbSetData [_idx, netId _group];
        _squadList lbSetColor [_idx, _color];
    };
} forEach DPCS_PLATOON_DATA;
